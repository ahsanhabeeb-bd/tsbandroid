...
class AvableDateForAdmin extends StatefulWidget {
  final String id;
  const AvableDateForAdmin({super.key, required this.id});

  @override
  State<AvableDateForAdmin> createState() => _AvableDateForAdminState();
}

class _AvableDateForAdminState extends State<AvableDateForAdmin> {
  ...
  Future<void> _loadId() async {
    String id = widget.id; // Get the 'id' value
    setState(() {
  ...
  Future<void> _saveDates() async {
    ...
    List<String> selectedDates = _selectedDates
        .map((date) =>
            DateFormat('yyyy-MM-dd').format(date)) // Format with leading zeros
        .toList();
    ...
    if (_id != 'Loading...') {
      FirebaseFirestore firestore = FirebaseFirestore.instance;
      try {
        // Save the dates in Firestore under the employee document
